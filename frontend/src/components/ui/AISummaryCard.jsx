...
export const AISummaryCard = ({
  ...
  return (
    <motion.div
      ...
      className="dash-card p-6 border-l-4 border-l-[#2563EB]"
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#64748B] font-medium">Grounded Local LLM Synthesis</p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="py-6 text-center space-y-2">
          ...
          <p className="text-xs text-[#64748B] font-medium">Synthesizing indicator data...</p>
        </div>
      ) : data ? (
        <div className="space-y-4">
          <div className="text-sm leading-relaxed text-[#0F172A] font-medium bg-[#F8FAFC] p-4 rounded-xl border border-[#E2E8F0]">
            "{data.summary}"
          </div>
            ...
            <div className="space-y-2 pt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {data.key_metrics.slice(0, 6).map((m, idx) => (
                  <div key={idx} className="p-3 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0] text-xs">
